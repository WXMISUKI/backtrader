...
CAPABILITY_ITEMS: List[CapabilityItem] = [
    ...
    CapabilityItem(
        ...
        category="report",
        primary_tool="execute_workflow",
        support_tools=["get_workflow_replay", "get_decision_session_replay", "get_workflow_learning_stats", "get_decision_session_stats", "generate_stock_report"],
        when_to_use=["用户要求复盘、总结执行效果、回看过程", "需要把回放、反馈、学习统计整理成复盘报告"],
        fallback_tools=["plan_collaboration", "answer_decision_request"],
        output_contract=["ok", "tool", "category", "data_source", "summary", "data", "meta"],
        notes="第三条标准工作流，收口型能力，优先复用回放和统计。",
    ),
    CapabilityItem(
