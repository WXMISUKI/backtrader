...
目标：把 Phase 82 的修复优先级继续往下接一层，只修 `production_gate` 和 `daily_execution_brief` 的稳定透传，不扩张新的分析层或新的编排层。

任务拆分：

| # | 任务 | 优先级 | 说明 |
|---|------|--------|------|
| S.1 | 编写 Phase 83 SDD | P0 | 明确关键透传修复边界 |
| S.2 | 修复 `production_gate` 透传 | P0 | 确保基线入口稳定可见 |
| S.3 | 修复 `daily_execution_brief` 透传 | P0 | 确保第一屏简报稳定可见 |
| S.4 | 更新验收规则 | P0 | 避免结构字段缺失误判 |
| S.5 | 补最小测试 | P1 | 固定字段存在/缺失两种场景 |
| S.6 | 编写归档文档 | P1 | 记录关键门禁修复闭环 |
| S.7 | 更新路线图与任务清单 | P1 | 固化关键透传口径 |
...
### 阶段 T：投产产物契约统一与 `latest.json` 写入修复

目标：把 Phase 83 的关键透传继续往下收一层，只统一 `latest.json` 的最终契约，让日常运行、验收和基线都以同一份主产物为准。

任务拆分：

| # | 任务 | 优先级 | 说明 |
|---|------|--------|------|
| T.1 | 编写 Phase 84 SDD | P0 | 明确产物契约边界 |
| T.2 | 在 pipeline 中写入关键契约字段 | P0 | `latest.json` 主契约 |
| T.3 | 收紧 daily_run 读取优先级 | P0 | 优先消费 `latest.json` |
| T.4 | 收紧 acceptance 契约检查 | P0 | 优先读取 `latest.json` |
| T.5 | 补最小测试 | P1 | 字段存在/缺失两种场景 |
| T.6 | 编写归档文档 | P1 | 记录产物契约统一闭环 |
| T.7 | 更新路线图与任务清单 | P1 | 固化 `latest.json` 口径 |
...
### 阶段 U：真实数据稳定性增强与 provider 可见性收紧

目标：把 Phase 84 的主契约继续往下收一层，把数据可信度和 provider 可见性统一下沉到健康预检、流水线、验收和基线入口。

任务拆分：

| # | 任务 | 优先级 | 说明 |
|---|------|--------|------|
| U.1 | 编写 Phase 85 SDD | P0 | 明确数据稳定性边界 |
| U.2 | 稳定输出 provider 视图 | P0 | 健康预检直接可读 |
| U.3 | 稳定透传 provider 与可信度 | P0 | 流水线与验收共用 |
| U.4 | 在基线入口突出 provider 与可信度 | P0 | 第一眼就能看到 |
| U.5 | 补最小回归测试 | P1 | 主源 / 备源 / 兜底 |
| U.6 | 编写归档文档 | P1 | 记录 provider 可见性闭环 |
| U.7 | 更新路线图与任务清单 | P1 | 固化数据可信度口径 |
...
- 数据降级与结构缺失能清晰区分。
- 低可信数据更倾向于观察或跳过，而不是强行动。
- 不破坏 `ready / caution / failed / blocked` 语义。

### 阶段 V：量产环境门禁与回归门禁收口

目标：把 `quant` 作为主开发和回归优先环境的约定，收成入口级执行门禁，避免错误环境影响日常判断。

任务拆分：

| # | 任务 | 优先级 | 说明 |
|---|------|--------|------|
| V.1 | 编写 Phase 86 SDD | P0 | 明确环境门禁边界 |
| V.2 | 增加环境自检工具 | P0 | 统一判断当前执行环境 |
| V.3 | 在 daily_run 中写入环境门禁 | P0 | `run_status` 可读 |
| V.4 | 在 acceptance 中检查环境门禁 | P0 | `latest.json` / `run_status` 可读 |
| V.5 | 在 baseline 中输出环境门禁 | P0 | 第一眼就能看见 |
| V.6 | 补最小回归测试 | P1 | 环境信息写入与读取 |
| V.7 | 编写归档文档 | P1 | 记录量产环境门禁闭环 |
| V.8 | 更新路线图与任务清单 | P1 | 固化 `quant` 口径 |
...
目标：把 `daily_run`、`acceptance` 和 `baseline` 收成一个薄的自动回归门禁，让日常关键验证可以一键完成，并把结果回写到现有产物里。

任务拆分：

| # | 任务 | 优先级 | 说明 |
|---|------|--------|------|
| W.1 | 编写 Phase 87 SDD | P0 | 明确自动回归门禁边界 |
| W.2 | 实现回归门禁入口 | P0 | `examples/daily_watchlist_regression_gate.py` |
| W.3 | 串联 `daily_run -> acceptance -> baseline` | P0 | 失败可跳过后续步骤 |
| W.4 | 回写回归结果 | P0 | `run_status` / `latest.json` |
| W.5 | 补最小回归测试 | P1 | 状态分类与回写 |
| W.6 | 编写归档文档 | P1 | 记录一键验证闭环 |
| W.7 | 更新路线图与任务清单 | P1 | 固化回归门禁口径 |

验收标准：

- 能一键跑完日常关键验证步骤。
- 结果可以区分 `pass / warn / block`。
- 回归结果能写回现有产物。
- 失败时能清楚知道卡在哪一步。
- 不破坏 `ready / caution / failed / blocked` 语义。
...
目标：把 Phase 87 的自动回归门禁继续往下收一层，让 `regression_gate` 进入 `daily_run`、`acceptance` 和 `baseline` 的默认消费协议，并把阻断时的修复提示收得更直接。

任务拆分：

| # | 任务 | 优先级 | 说明 |
|---|------|--------|------|
| X.1 | 编写 Phase 88 SDD | P0 | 明确默认化边界 |
| X.2 | 默认化 `regression_gate` 视图 | P0 | 共享协议 |
| X.3 | 在 daily_run 中写入回归门禁 | P0 | `run_status` |
| X.4 | 在 acceptance 中检查回归门禁 | P0 | 可验字段 |
| X.5 | 在 baseline 中突出回归门禁 | P0 | 第一眼可见 |
| X.6 | 补最小回归测试 | P1 | 默认读取与回写 |
| X.7 | 编写归档文档 | P1 | 记录默认化闭环 |
| X.8 | 更新路线图与任务清单 | P1 | 固化回归门禁口径 |
...
### 阶段 Y：日常使用测试规范化与最小对话入口说明

目标：把项目的“怎么用”收成一份明确、可重复的日常使用协议，并把当前是否存在前端对话入口说明清楚，避免用户在入口上反复试错。

任务拆分：

| # | 任务 | 优先级 | 说明 |
|---|------|--------|------|
| Y.1 | 编写 Phase 89 SDD | P0 | 明确日常使用测试边界 |
| Y.2 | 固化日常使用测试顺序 | P0 | 数据健康 -> 决策 -> 回看 -> 验收 -> 门禁 |
| Y.3 | 写清最小入口路径 | P0 | CLI / HTTP API / 日常脚本 |
| Y.4 | 说明前端对话入口现状 | P0 | 明确当前没有独立 Web Chat UI |
| Y.5 | 更新 README_QUANT 使用说明 | P0 | 供日常直接照着跑 |
| Y.6 | 补最小验证步骤 | P1 | 只做轻量验证，不扩展新平台 |
| Y.7 | 编写归档文档 | P1 | 记录使用协议收口 |
| Y.8 | 更新路线图与任务清单 | P1 | 固化日常使用口径 |
...
目标：把 Phase 89 的使用协议、入口说明和最小运行结果收成正式归档，让日常使用入口不再停留在临时说明层。

任务拆分：

| # | 任务 | 优先级 | 说明 |
|---|------|--------|------|
| Z.1 | 编写 Phase 89 归档文档 | P0 | 记录日常使用协议收口 |
| Z.2 | 更新路线图状态 | P0 | 把 Phase 89 标记为完成 |
| Z.3 | 更新任务清单状态 | P0 | 固化使用入口口径 |
| Z.4 | 检查 README 链接完整性 | P1 | 确认入口说明可直接使用 |
